    ...
    try:
        ...
        with conn.cursor() as cur:
            if timezone:
                ...
                cur.execute(
                    "SELECT name, abbreviation, capital, timezone FROM states ORDER BY name"
                )
            rows = cur.fetchall()
        conn.close()
        return jsonify(
            states=[
                {"name": r[0], "abbreviation": r[1], "capital": r[2], "timezone": r[3]}
                for r in rows
            ]
        )
        ...
        return jsonify(
            {
                ...
                "capital": capital,
                "timezone": timezone,
                "top_cities": [{"name": c[0], "population": c[1]} for c in cities],
            }
        )
    except Exception as e:
        return jsonify(error=str(e)), 500
